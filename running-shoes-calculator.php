    ...
    <script>
      (() => {
        ...
        const getSafeCtaUrl = () => {
          ...
          try {
            const parsed = new URL(cta.url, window.location.origin);
        ...
        const saveState = () => {
          try {
            ...
            console.warn('Unable to persist calculator state.', e);
          }
        };
        ...
        form.addEventListener('submit', (event) => {
          ...
          if (Number.isNaN(weeklyDistance) || weeklyDistance < 0) {
            resultDiv.classList.remove('hidden');
            resultDiv.innerHTML = '<div class="alert">Please enter a valid non-negative weekly distance.</div>';
            weeklyInput.focus();
            return;
          }
          ...
          const safeCtaUrl = getSafeCtaUrl();
          const ctaMarkup = shouldRenderCta(health.key) && safeCtaUrl
            ? `<a class="cta" target="_blank" rel="noopener sponsored nofollow" href="${safeCtaUrl}">${escapeHtml(cta.label || 'Find replacement shoes')}</a>`
            : '';
